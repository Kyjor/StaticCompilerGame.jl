...
libsdl2_image::String = joinpath(pwd(), "lib", "image", "libSDL2_image-2.0.so")  
...
function Base.getproperty(x::Ptr{SDL_RWops}, f::Symbol)
    ...
    f === :hidden && return Ptr{Cvoid}(x + 48)
    return getfield(x, f)
end
...
struct SDL_Event end

function Base.getproperty(x::Ptr{SDL_Event}, f::Symbol)
    f === :type && return unsafe_load(Ptr{UInt32}(x))
    f === :common && return unsafe_load(Ptr{SDL_CommonEvent}(x))
    f === :display && return unsafe_load(Ptr{SDL_DisplayEvent}(x))
    f === :window && return unsafe_load(Ptr{SDL_WindowEvent}(x))
    f === :key && return unsafe_load(Ptr{SDL_KeyboardEvent}(x))
    f === :edit && return unsafe_load(Ptr{SDL_TextEditingEvent}(x))
    f === :editExt && return unsafe_load(Ptr{SDL_TextEditingExtEvent}(x))
    f === :text && return unsafe_load(Ptr{SDL_TextInputEvent}(x))
    f === :motion && return unsafe_load(Ptr{SDL_MouseMotionEvent}(x))
    f === :button && return unsafe_load(Ptr{SDL_MouseButtonEvent}(x))
    f === :wheel && return unsafe_load(Ptr{SDL_MouseWheelEvent}(x))
    f === :jaxis && return unsafe_load(Ptr{SDL_JoyAxisEvent}(x))
    f === :jball && return unsafe_load(Ptr{SDL_JoyBallEvent}(x))
    f === :jhat && return unsafe_load(Ptr{SDL_JoyHatEvent}(x))
    f === :jbutton && return unsafe_load(Ptr{SDL_JoyButtonEvent}(x))
    f === :jdevice && return unsafe_load(Ptr{SDL_JoyDeviceEvent}(x))
    f === :jbattery && return unsafe_load(Ptr{SDL_JoyBatteryEvent}(x))
    f === :caxis && return unsafe_load(Ptr{SDL_ControllerAxisEvent}(x))
    f === :cbutton && return unsafe_load(Ptr{SDL_ControllerButtonEvent}(x))
    f === :cdevice && return unsafe_load(Ptr{SDL_ControllerDeviceEvent}(x))
    f === :ctouchpad && return unsafe_load(Ptr{SDL_ControllerTouchpadEvent}(x))
    f === :csensor && return unsafe_load(Ptr{SDL_ControllerSensorEvent}(x))
    f === :adevice && return unsafe_load(Ptr{SDL_AudioDeviceEvent}(x))
    f === :sensor && return unsafe_load(Ptr{SDL_SensorEvent}(x))
    f === :quit && return unsafe_load(Ptr{SDL_QuitEvent}(x))
    f === :user && return unsafe_load(Ptr{SDL_UserEvent}(x))
    f === :syswm && return unsafe_load(Ptr{SDL_SysWMEvent}(x))
    f === :tfinger && return unsafe_load(Ptr{SDL_TouchFingerEvent}(x))
    f === :mgesture && return unsafe_load(Ptr{SDL_MultiGestureEvent}(x))
    f === :dgesture && return unsafe_load(Ptr{SDL_DollarGestureEvent}(x))
    f === :drop && return unsafe_load(Ptr{SDL_DropEvent}(x))
    f === :padding && return unsafe_load(Ptr{NTuple{56, UInt8}}(x))
    return getfield(x, f)
end
